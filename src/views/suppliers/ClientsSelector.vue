<template>
    <div>
        <label class="block text-gray-800 font-sm text-sm mb-1">Clientes</label>
        <div class="relative flex items-center gap-2">
            <input
                v-model="search"
                @focus="abrirModalClientes"
        ...
        <ModalReutilizable :show="openModal" @close="openModal = false">
            <div>
                <h2 class="text-lg font-semibold text-gray-800 mb-2">Seleccionar Cliente</h2>
                ...
                <div class="space-y-2 mb-4" v-if="!filteredClientes.length">
                    ...
                    <input v-model="nuevoCliente.email" placeholder="Correo electrónico" class="w-full border px-3 py-2 rounded" />
                    ...
                    <input v-model="nuevoCliente.contactName" placeholder="Contacto de referencia" class="w-full border px-3 py-2 rounded" />
...
import { createClient, getMyClients, updateClient } from '../../services/suppliers.service'
...
// Cargar clientes solo al abrir el modal
async function cargarClientes() {
    if (clientesCargados.value) return
    try {
        const suppliers = await getMyClients()
